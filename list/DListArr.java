...
public class DListArr extends ListArr {
   ...
   **/
  protected DListNodeArr newNode(Object[] item, DListArr list,
                              DListNodeArr prev, DListNodeArr next) {
    return new DListNodeArr(item, list, prev, next);
  }
  ...
  public DListArr() {
    ...
    public DListArr(DListArr copy) {
        try {
            head = new DListNodeArr();
            head.next = head;
            head.prev = head;
            size=0;
            DListNodeArr curr = (DListNodeArr) copy.front();
            for (int i=0; i<copy.length(); i++) {
                if (i!=0) {
                    curr = (DListNodeArr) curr.next();
                }
                this.insertBack(curr.item());
            }
        }
        catch (InvalidNodeException e1) {
            System.out.println("error in DListArr constructor");
  ...
  public void insertFront(Object[] item) {
      DListNodeArr temp = newNode(item,this,head,head.next);
      head.next.prev = temp;
      head.next = temp;
      size++;
    // Your solution here.  Similar to Homework 4, but now you need to specify
  ...
  public void insertBack(Object[] item) {
    // Your solution here.  Similar to Homework 4, but now you need to specify
    //   the `list' field (second parameter) as well.
      DListNodeArr temp = newNode(item,this,head.prev,head);
      head.prev.next = temp;
      head.prev = temp;
      size++;
  }
  ...
  public String toString() {
    String result = "[  ";
    DListNodeArr current = head.next;
    while (current != head) {
